...
Используйте Node.js 24.14.0 из `.node-version` и устанавливайте зависимости командой `npm ci`.

## CI

Для push в `imurd` и PR с целевой веткой `imurd` запускаются три проверки:
...
Тест БД запускается командой `npm run test:db` с отдельной тестовой `DATABASE_URL` и `ALLOW_DB_TESTS=1`. Он создаёт и удаляет только собственную тестовую запись. Не используйте production-базу.
