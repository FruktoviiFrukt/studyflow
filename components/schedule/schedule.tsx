  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-5">
      ...
      <Card className="overflow-hidden rounded-2xl border-gray-200 bg-white shadow-sm">
        ...
        {view === "week" ? (
          <WeekGrid days={days} lessons={lessons} today={today} />
        ) : (
      ...
      >
        {Object.entries(subjects).map(([id, subject]) => (
          ...
          >
            <span
              aria-hidden="true"
              className={cn("size-2 rounded-full", subject.dot)}
            />
            {subject.name}
          </span>
        ))}
      </section>
      <p className="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50/50 p-4 text-xs leading-5 text-gray-500">
        <Info
          aria-hidden="true"
          className="mt-0.5 size-4 shrink-0 text-blue-500"
        />
        Демонстрационные данные. Занятия повторяются каждую неделю.
      </p>
    </div>
  );
}
