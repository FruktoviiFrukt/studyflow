...
import { filterSubjects, subjectGrade, type Semester, type SemesterFilter, calculateOverall, calculateStage, createStage, createSubject, defaultFormula, gradeStatus, initialSubjects, isLowGrade, type GradeStage, type GradeSubject } from "@/lib/grades";
    ...
    <div className="mx-auto max-w-[1440px] space-y-6">
      ...
      <Card className="rounded-2xl border-blue-100 bg-gradient-to-br from-blue-50 via-white to-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="max-w-sm text-sm leading-6 text-gray-500"><p className="font-medium text-gray-700">{subject.name} · Семестр {subject.semester}</p><p>Рассчитано этапов: {completed.length} из {results.length}</p><p>Среднее результатов рассчитанных этапов. Пока заполнены не все этапы, результат предварительный.</p></div>
        </div>
      </Card>
