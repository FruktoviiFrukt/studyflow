...
test("five flat stages use 15/15/15/15/40 weights", () => {
  ...
  assert.equal(
    calculateSemester(subject(["10", "10", "10", "10", "10"])).value,
    10,
  );
});
test("custom formulas support percentages, decimals, parentheses and precedence", () => {
  ...
  assert.equal(
    calculateSemester(subject(undefined, "g1 * 0,5 + g2 * 50%")).value,
    7,
  );
  assert.equal(calculateSemester(subject(undefined, "g1 - g2 / 2")).value, 5);
  assert.ok(
    Math.abs(calculateSemester(subject(undefined, "")).value - 7.8) < 1e-10,
  );
});
test("missing required grades do not produce partial or zero results", () => {
  assert.equal(
    calculateSemester(subject(["8", "6", "10", "4", ""])).value,
    null,
  );
  assert.equal(
    calculateSemester(subject(["8", "", "", "", ""], "g1")).value,
    8,
  );
  assert.equal(
    calculateOverall([createSubject("empty", "Empty")]).average,
    null,
  );
});
test("invalid grades and unsafe or invalid formulas are rejected", () => {
  for (const formula of [
    ...
    assert.ok(calculateSemester(subject(undefined, formula)).error, formula);
  for (const grade of ["0", "11", "abc", "0x8", "-1"])
    assert.ok(
      calculateSemester(subject([grade, "6", "10", "4", "9"])).error,
      grade,
    );
});
...
test("warnings use unrounded individual grades independently of the final result", () => {
  ...
  for (const value of ["5", "", "0", "invalid"])
    assert.equal(isLowGrade(value), false);
});
