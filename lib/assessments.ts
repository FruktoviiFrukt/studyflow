...
export function getDemoAssessments(
  ...
  return subjects
    .slice(0, kind === "Переаттестация" ? 2 : 5)
    .map((subject, day) => ({
      id: `${kind}-${week}-${day}`,
      date: addDays(week, (day + index) % 5),
      slot: (day + index) % 3,
      subject,
      type: kind,
      classroom: `3-${201 + day}`,
      ...
      topic:
        kind === "Экзамен"
          ? "Итоговая проверка знаний за семестр"
          : kind === "Переаттестация"
            ? "Повторная проверка пройденного материала"
            : `Проверка знаний · ${kind.toLowerCase()}`,
    }));
}
