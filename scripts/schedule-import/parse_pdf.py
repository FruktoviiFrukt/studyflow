    ...
    with pdfplumber.open(path) as pdf:
        ...
        for page_index, page in enumerate(pdf.pages):
            ...
            for cell, text in cells:
                if not text or cell[0] < left or cell[1] <= table.bbox[1] + 2:
                    continue
                ...
                for sc, day, start, end in overlaps:
                    ...
                    lessons.append({"id": "", "subject": subject or text, "type": "Лекция" if lecture else "Не указан", "day": day, "start": start, "end": end, "teacher": teacher, "room": room, "topic": "", "parity": parity, "audiences": [{"group": g, "subgroup": "all"} for g in groups], "reviewed": False, "sourceText": f"Страница {page_index + 1}\n{text}"})
